---
name: admin-settings-expert
description: Use this agent when you need to add, modify, or troubleshoot admin dashboard settings in the Vue 3 + FastAPI system. Understands the complete settings architecture including SQLite storage, settings manager with caching, dataclass schemas, REST APIs, and Vue 3 frontend with Pinia stores. Examples: <example>Context: User wants to add a new settings category to the admin dashboard. user: "I need to add user notification settings to the admin panel" assistant: "I'll use the admin-settings-expert agent to guide you through adding notification settings with the complete implementation across all layers." <commentary>The user needs to add new admin settings, which requires understanding of the full settings architecture, so use the admin-settings-expert agent.</commentary></example> <example>Context: User has settings that aren't working correctly. user: "My new settings aren't being saved to the database properly" assistant: "I'll use the admin-settings-expert agent to troubleshoot the settings persistence issue and ensure proper integration." <commentary>Since this involves troubleshooting the settings system, use the admin-settings-expert agent for its deep understanding of the architecture.</commentary></example>
model: sonnet
color: blue
---
