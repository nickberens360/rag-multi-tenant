...
from .tenant_prompt_builder import get_prompt_builder
...
# NOTE: These are now generic templates. Tenant-specific prompts are generated by TenantPromptBuilder
DEFAULT_PROMPTS = {
    "system_template": """You are an AI assistant. You help visitors learn about the organization's
background, skills, experience, and interests. Use the following pieces of context to
...
def _build_dynamic_system_prompt(tenant_id: Optional[str] = None) -> str:
    """
    Build system prompt with tenant-aware customization and response settings guidance.

    Args:
        tenant_id: UUID of the tenant for customization (optional, falls back to generic)
    ...
    try:
        ...
        # Build tenant-specific base prompt using TenantPromptBuilder
        prompt_builder = get_prompt_builder()

        if tenant_id:
            try:
                tenant_uuid = UUID(tenant_id)
                # Get tenant-aware base prompt (without context - we'll add that later)
                base_prompt = prompt_builder.build_system_prompt(
                    tenant_id=tenant_uuid, prompt_type="system", context="{context}"  # Placeholder for LangChain
                )

                # Get tenant data to customize the "not found" message
                tenant_data = prompt_builder._get_tenant(tenant_uuid)
                org_name = tenant_data.get("name", "the organization")

            except (ValueError, Exception) as e:
                logger.warning(f"Failed to build tenant-specific prompt for {tenant_id}: {e}, using generic")
        ...
        enhanced_prompt = (
            ...
            f'"I don\'t have that information in my knowledge base about {org_name}."'
            "\n"
            "**DO NOT:**"
            ...
            "\n"
            "1.  **Persona:** When the user asks about 'you' or 'your' experience, respond about the organization appropriately."
            "\n"
            "2.  **Resume/Profile Requests:** If asked for a resume or profile, synthesize the "
            "provided context into a clear, professional summary. **NEVER** state that you are an AI "
            "or do not have information. Use the context provided."
            "\n"
        ...
        return DEFAULT_PROMPTS.get("system_template", "")


def create_qa_chain(llm: BaseLanguageModel, tenant_id: Optional[str] = None) -> Runnable:
    """
    Creates the main question-answering chain with dynamic response settings.

    Args:
        llm: Language model instance
        tenant_id: Optional tenant UUID for customized prompts

    Returns:
        Runnable chain for question answering
    """
    system_prompt = _build_dynamic_system_prompt(tenant_id=tenant_id)
...
async def stream_with_fallback(
    ...
    for llm_name, llm_instance in llm_order:
        ...
        try:
            ...
            qa_chain = create_qa_chain(llm_instance, tenant_id=tenant_id)
                        ...
                        try:
                            ...
                            if env_key:
                                ...
                                try:
                                    ...
                                    qa_env = create_qa_chain(fallback_llm, tenant_id=tenant_id)
                            ...
                            if gem_llm is not None:
                                qa_gem = create_qa_chain(gem_llm, tenant_id=tenant_id)
